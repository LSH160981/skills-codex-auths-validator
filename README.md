...
- Validate `*.json` under `/home/docker/CLIProxyAPI/auths`
- Verify credentials via `https://chatgpt.com/backend-api/wham/usage`
- Keep/remove credentials based on strict rules
- Support ZIP batch import and import only passed files
- Support hourly auto-cleanup
- Support daily 00:00 GitHub learning checks and rule evolution

### Validation Rules

- `200 / 429`: keep (`429` is treated as rate/quota, not token death)
- `401 / 403`: invalid, delete
- malformed JSON / missing required fields: delete
- `._*.json` (AppleDouble junk files): delete
...
- Extract ZIP
- Validate JSON files with the same rules
- Import only passed files into `/home/docker/CLIProxyAPI/auths`
- Delete/quarantine failed files and produce a report

### Required Scheduled Jobs (2)

On a new machine, this skill must auto-provision and keep these jobs consistent:

1. **Hourly auth validation cleanup (Asia/Shanghai)**
...
- Core rules and evolution notes live in:
  - `skills/codex-auths-validator/SKILL.md`
  - `skills/codex-auths-validator/WORKFLOW.md`
- Git commit messages are maintained in Chinese.
