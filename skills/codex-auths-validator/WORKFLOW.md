...
- TG 先发**精简摘要**（不刷屏，从 `reports/hourly-reconcile-*.json` 读取关键统计，避免解析文本误判）；若 `auths` 与 `auths_no_quota` 两个目录都为空（以目录内 `*.json` 文件数判定，避免解析输出误判），则只发**极简通知**；异常/无效/临时错误时再自动附详细日志文件
