...
根据用户提供的两张运行截图（OCR 提取）整理出以下关键样例，作为文档证据：

### 样例 A：ZIP 导入后分层结果

- 总 JSON：500
- 导入 `auths`（有效有额度）：494
- 导入 `auths_no_quota`（有效无额度）：6
- 移入 `auths_invalid`（无效）：0
...
- 有额度约 5745~5758
- 无额度约 68~72
- 删除 0~9（常见原因：`auth_401` / `auth_403` / `status_500`）

说明：该类波动在历史上主要由并发重叠与临时错误引发。当前已通过“并发锁 + 临时错误保留 + 双目录稳定分层”策略进行抑制。
